...
void zip_high_scores() {
  ...
  for (const auto &high_score : high_scores) {
    ...
    if (!equal_high_score) new_high_scores.push_back(high_score);
  }
...
int read_high_scores_from_file() {
  ...
  if (!in_file.is_open()) {
    std::cout << "Filed to open file for read " << high_sore_filename
              << std::endl;
    return -1;
  }
...
void add_or_update_high_score(const std::string &name, const int attempts) {
  ...
  for (auto &high_score : high_scores) {
    if (high_score.first == name) {
      equal_name = true;
      if (high_score.second > attempts) high_score.second = attempts;
    }
  }
  if (!equal_name) high_scores.push_back({name, attempts});
}
int save_score(const std::string &name, const int attempts) {
  ...
  if (!out_file.is_open()) {
    std::cout << "Failed to open file for write" << high_sore_filename
              << std::endl;
    return -1;
  }
  for (const auto &high_score : high_scores) {
